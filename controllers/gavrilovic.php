...
class Gavrilovic{
 ...
 public function metodProracunaGavrilovic($id_user,$imesliva, $t, $Hgod, $y1, $py1, $y2, $py2, $y3, $py3, $xa1, $pxa1, $xa2, $pxa2, $xa3, $pxa3, $f1, $pf1, $f2, $pf2, $f3, $pf3, $Jsr, $Fs, $O, $Nsr, $Nu, $Ls,$image_name){ 
    //  $model_gav_old=[$imesliva,$Nsr,$Jsr,$t,$Ls,$O,$Nu,$Fs,$y,$xa,$f,$Hgod];
    //  var_dump($imesliva);
    ...
    //   var_dump($f,$f1,$pf1,$f2,$pf2,$f3,$pf3);
       echo"<h5 class='font-italic' style='font-size: 15px;'>Koeficijent Y = $y srednja vrednost za dati sliv,</h5>";
       ...
       $Zk=$y*$xa*($f+sqrt($Jsr));
          ...
          if($Zk >= 0.01 && $Zk <= 0.10){
             ...
             echo "<h6 class='font-italic text-primary' style='font-size: 15px;'>V kategorija razornosti ".number_format($Zk, 2, '.', '')." - vrlo slaba erozija tragovi erozije</h6>";
    ...
    session_start();
      ...
      $dao= new DAO();
      $allname=$dao->getAllGavrilovicResult($id_user);
    if(!empty($allname)){
        foreach($allname as $value){
            if($value['imesliva']!==$imesliva){
              $dao->insertGavrilovic($id_user,$imesliva,$t, $Hgod, $y,$y1, $py1, $y2, $py2, $y3, $py3, $xa,$xa1, $pxa1, $xa2, $pxa2, $xa3, $pxa3, $f, $f1, $pf1, $f2, $pf2, $f3, $pf3, $Jsr, $Zk, $Fs, $O, $Nsr, $Nu, $Ls, $W, $R, $Gg, $Ggkm,$image_name);
              break;
            }else{
                echo"<h6 class='font-italic text-danger' style='font-size: 15px;'>pod istim imenom je vec izvrsen upis u bazu</h6>";
                break;
            }
        }
    }else{
